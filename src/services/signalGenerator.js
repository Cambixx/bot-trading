import { performTechnicalAnalysis, calculateOrderBookMetrics } from './technicalAnalysis.js';
...
export function generateSignal(analysis, symbol, multiTimeframeData = {}, mode = 'BALANCED') {
    ...
    const { indicators = {}, levels = {}, patterns = {}, volume = {}, price, regime, choppiness } = analysis;
    const buyerPressure = analysis.buyerPressure ?? volume.buyerPressure ?? null;
    ...
    const orderBook = analysis.orderBook || null;
    const obMetrics = orderBook ? calculateOrderBookMetrics(orderBook, 10) : null;
    const topBookLevels = orderBook ? {
        bids: Array.isArray(orderBook.bids) ? orderBook.bids.slice(0, 5).map(([p, q]) => [Number(p), Number(q)]) : [],
        asks: Array.isArray(orderBook.asks) ? orderBook.asks.slice(0, 5).map(([p, q]) => [Number(p), Number(q)]) : []
    } : null;

    const spreadLimitBps = mode === 'CONSERVATIVE' ? 8 : mode === 'BALANCED' ? 12 : mode === 'RISKY' ? 18 : 20;
    const minDepthNotional = mode === 'CONSERVATIVE' ? 50000 : mode === 'BALANCED' ? 30000 : mode === 'RISKY' ? 15000 : 10000;

    if (!obMetrics) {
        if (mode === 'CONSERVATIVE' || mode === 'BALANCED') return null;
        warnings.push({ text: 'Order Book no disponible', type: 'risk' });
    } else {
        if (obMetrics.spreadBps > spreadLimitBps) {
            if (mode === 'CONSERVATIVE' || mode === 'BALANCED') return null;
            warnings.push({ text: `Spread alto (${obMetrics.spreadBps.toFixed(1)} bps)`, type: 'risk' });
        }
        if (obMetrics.depthNotionalTopN < minDepthNotional) {
            if (mode === 'CONSERVATIVE' || mode === 'BALANCED') return null;
            warnings.push({ text: 'Poca profundidad (Order Book)', type: 'risk' });
        }
    }
    ...
    if (mode === 'CONSERVATIVE') {
        if (currentRegime !== 'TRENDING_BULL' && currentRegime !== 'TRENDING_BEAR') return null;
    }
    ...
    if (obMetrics) {
        if (obMetrics.spreadBps <= 4) finalNormalized += 0.04;
        else if (obMetrics.spreadBps >= spreadLimitBps * 0.9) finalNormalized -= 0.05;

        const obiAligned = (signalType === 'BUY' && obMetrics.obi >= 0.08) || (signalType === 'SELL' && obMetrics.obi <= -0.08);
        if (obiAligned) {
            finalNormalized += 0.06;
            reasons.push({ text: 'Order Book favorable', weight: 6 });
        } else {
            finalNormalized -= 0.03;
        }
    }
    ...
    return {
        ...
        indicators: {
            ...
            cvd20: indicators.cvd20 != null ? Number(indicators.cvd20.toFixed(2)) : null
        },
        execution: obMetrics ? {
            spreadBps: Number(obMetrics.spreadBps.toFixed(1)),
            obi: Number(obMetrics.obi.toFixed(3)),
            depthNotionalTopN: Math.round(obMetrics.depthNotionalTopN),
            bookTop: topBookLevels
        } : null,
        regime: currentRegime
    };
}

export function analyzeMultipleSymbols(symbolsData, multiTimeframeData = {}, mode = 'BALANCED', orderBooksBySymbol = {}) {
    const signals = [];
    ...
    for (const [symbol, candleData] of Object.entries(symbolsData)) {
        ...
        try {
            ...
            const orderBook = orderBooksBySymbol?.[symbol]?.data || null;
            const analysisWithOrderBook = orderBook ? { ...analysis, orderBook } : analysis;
            const mtfData = multiTimeframeData[symbol] || null;
            const signal = generateSignal(analysisWithOrderBook, symbol, mtfData, mode);
